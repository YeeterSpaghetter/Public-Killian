...
def fight(enemy):
    ...
    while eHealth > 0: # continues fighting if enemy is alive
        while turn == True: # allows users to make their decision 
            ...
            attButton = Button(fightWindow, text= 'Attack', command = lambda :att(health, enemy, eHealth)) # command to attack
            attButton.grid(row=1, column=0)
            
            magButton = Button(fightWindow, text= 'Magic', command = lambda :mag(health, enemy, eHealth)) # command to use magic
            magButton.grid(row=1, column=1)
            
            invButton = Button(fightWindow, text= 'Inventory', command = lambda :inv(health, energy)) # command to use the item
            invButton.grid(row=1, column=3)
            
            fleButton = Button(fightWindow, text= 'Flee', command = lambda :flee(enemy, boots)) # command to end the fight
